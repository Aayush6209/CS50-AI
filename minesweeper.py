...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.safes.add(cell)
        # print("Adding to safe in add_knowlwdge(1) ", cell)
        for old_sentence in self.knowledge:
            if cell in old_sentence.cells:
                old_sentence.cells.remove(cell)
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                continue
            for old_sentence in self.knowledge:
                if len(old_sentence.cells) == 0:
                    continue
                if old_sentence == sentence:
                    continue
                if old_sentence.cells.issubset(sentence.cells):
                    sentence.cells = sentence.cells.difference(
                        old_sentence.cells)
                    sentence.count = sentence.count-old_sentence.count
                elif sentence.cells.issubset(old_sentence.cells):
                    old_sentence.cells = cells = old_sentence.cells.difference(
                        sentence.cells)
                    old_sentence.count = old_sentence.count-sentence.count
        ...
        for new_sentence in self.knowledge:
            ...
            while len(safes) > 0:
                safe = safes.pop()
                self.mark_safe(safe)
        # removing unwanted sentences from knowledge base
        remove = []
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
